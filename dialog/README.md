# Dialog

Back in my day we used to just put this sort of content on a separate page.

```twig {"preview":true}
<div style="min-height: 275px">
    <twig:Dialog id="refund-payment">
        <twig:block name="button">
            <twig:Button type="button" {{ ...dialog_open_attrs }}>
                Refund payment
            </twig:Button>
        </twig:block>
        <twig:Dialog:Title>Refund payment</twig:Dialog:Title>
        <twig:Dialog:Description>
            The refund will be reflected in the customer’s bank account 2 to 3 business days after processing.
        </twig:Dialog:Description>
        <twig:Dialog:Body>
            <twig:Fieldset:Field>
                <twig:Fieldset:Label for="amount">Amount</twig:Fieldset:Label>
                <twig:Input id="amount" name="amount" placeholder="$0.00" />
            </twig:Fieldset:Field>
        </twig:Dialog:Body>
        <twig:Dialog:Actions>
            <twig:Button plain {{ ...dialog_close_attrs }}>
                Cancel
            </twig:Button>
            <twig:Button {{ ...dialog_close_attrs }}>Refund</twig:Button>
        </twig:Dialog:Actions>
    </twig:Dialog>
</div>
```

## Installation

::: installation

## Examples

### Basic example

Use the `Dialog`, `Dialog:Title`, `Dialog:Description`, `Dialog:Body`, and `Dialog:Actions` components to build a dialog:

```twig {"preview":true}
<div style="min-height: 275px">
    <twig:Dialog id="refund-payment">
        <twig:block name="button">
            <twig:Button type="button" {{ ...dialog_open_attrs }}>
                Refund payment
            </twig:Button>
        </twig:block>
        <twig:Dialog:Title>Refund payment</twig:Dialog:Title>
        <twig:Dialog:Description>
            The refund will be reflected in the customer’s bank account 2 to 3 business days after processing.
        </twig:Dialog:Description>
        <twig:Dialog:Body>
            <twig:Fieldset:Field>
                <twig:Fieldset:Label for="amount">Amount</twig:Fieldset:Label>
                <twig:Input id="amount" name="amount" placeholder="$0.00" />
            </twig:Fieldset:Field>
        </twig:Dialog:Body>
        <twig:Dialog:Actions>
            <twig:Button plain {{ ...dialog_close_attrs }}>
                Cancel
            </twig:Button>
            <twig:Button {{ ...dialog_close_attrs }}>Refund</twig:Button>
        </twig:Dialog:Actions>
    </twig:Dialog>
</div>
```

### Dialog width

Use the `size` prop on the `Dialog` component to control the max-width of the dialog:

```twig {"preview":true}
<div style="min-height: 275px">
    <twig:Dialog id="refund-payment" size="xl">
        <twig:block name="button">
            <twig:Button type="button" {{ ...dialog_open_attrs }}>
                Refund payment
            </twig:Button>
        </twig:block>
        <twig:Dialog:Title>Refund payment</twig:Dialog:Title>
        <twig:Dialog:Description>
            The refund will be reflected in the customer’s bank account 2 to 3 business days after processing.
        </twig:Dialog:Description>
        <twig:Dialog:Body>
            <twig:Fieldset:Field>
                <twig:Fieldset:Label for="amount">Amount</twig:Fieldset:Label>
                <twig:Input id="amount" name="amount" placeholder="$0.00" />
            </twig:Fieldset:Field>
        </twig:Dialog:Body>
        <twig:Dialog:Actions>
            <twig:Button plain {{ ...dialog_close_attrs }}>
                Cancel
            </twig:Button>
            <twig:Button {{ ...dialog_close_attrs }}>Refund</twig:Button>
        </twig:Dialog:Actions>
    </twig:Dialog>
</div>
```

Available size options include `xs`, `sm`, `md`, `lg`, `xl`, `2xl`, `3xl`, `4xl`, and `5xl`.

### Opening from dropdown

When opening a `Dialog` from a dropdown menu, make sure the dialog is rendered outside of the dropdown:

```twig {"preview":true}
<div style="min-height: 250px">
    <twig:Dialog id="refund-from-dropdown">
        <twig:block name="button">
            <twig:Dropdown>
                <twig:Dropdown:Button outline>
                    Options
                    <twig:ux:icon name="heroicons:chevron-down-16-solid" />
                </twig:Dropdown:Button>
                <twig:Dropdown:Menu anchor="bottom start">
                    <twig:Dropdown:Item {{ ...dialog_open_attrs }}>
                        <twig:Dropdown:Label>Refund</twig:Dropdown:Label>
                    </twig:Dropdown:Item>
                    <twig:Dropdown:Item href="#" disabled>
                        <twig:Dropdown:Label>Download</twig:Dropdown:Label>
                    </twig:Dropdown:Item>
                </twig:Dropdown:Menu>
            </twig:Dropdown>
        </twig:block>
        <twig:Dialog:Title>Refund payment</twig:Dialog:Title>
        <twig:Dialog:Description>
            The refund will be reflected in the customer's bank account 2 to 3 business days after processing.
        </twig:Dialog:Description>
        <twig:Dialog:Body>
            <twig:Fieldset:Field>
                <twig:Fieldset:Label for="amount">Amount</twig:Fieldset:Label>
                <twig:Input id="amount" name="amount" placeholder="$0.00" />
            </twig:Fieldset:Field>
        </twig:Dialog:Body>
        <twig:Dialog:Actions>
            <twig:Button plain {{ ...dialog_close_attrs }}>Cancel</twig:Button>
            <twig:Button {{ ...dialog_close_attrs }}>Refund</twig:Button>
        </twig:Dialog:Actions>
    </twig:Dialog>
</div>
```

### Auto-focusing elements

Add the `autofocus` attribute to any form control or button in the dialog to automatically focus it when the dialog opens:

```twig {"preview":true}
<div style="min-height: 275px">
    <twig:Dialog id="refund-payment">
        <twig:block name="button">
            <twig:Button type="button" {{ ...dialog_open_attrs }}>
                Refund payment
            </twig:Button>
        </twig:block>
        <twig:Dialog:Title>Refund payment</twig:Dialog:Title>
        <twig:Dialog:Description>
            The refund will be reflected in the customer’s bank account 2 to 3 business days after processing.
        </twig:Dialog:Description>
        <twig:Dialog:Body>
            <twig:Fieldset:Field>
                <twig:Fieldset:Label for="amount">Amount</twig:Fieldset:Label>
                <twig:Input id="amount" name="amount" placeholder="$0.00" autofocus />
            </twig:Fieldset:Field>
        </twig:Dialog:Body>
        <twig:Dialog:Actions>
            <twig:Button plain {{ ...dialog_close_attrs }}>
                Cancel
            </twig:Button>
            <twig:Button {{ ...dialog_close_attrs }}>Refund</twig:Button>
        </twig:Dialog:Actions>
    </twig:Dialog>
</div>
```

### With scrolling content

Dialogs automatically become scrollable if their content is taller than the viewport:

```twig {"preview":true}
<div style="min-height: 275px">
    <twig:Dialog id="agree-to-terms" size="xl">
        <twig:block name="button">
            <twig:Button type="button" {{ ...dialog_open_attrs }}>
                Agree to terms
            </twig:Button>
        </twig:block>
        <twig:Dialog:Title>Terms and conditions</twig:Dialog:Title>
        <twig:Dialog:Description>
            Please agree to the following terms and conditions to continue.
        </twig:Dialog:Description>
        <twig:Dialog:Body class="text-sm/6 text-zinc-900 dark:text-white">
            <p class="mt-4">
                By accessing and using our services, you are agreeing to these terms, which have been meticulously tailored
                for our benefit and your compliance.
            </p>
            <h3 class="mt-6 font-bold">Comprehensive Acceptance of Terms</h3>
            <p class="mt-4">
                Your engagement with our application signifies your irrevocable acceptance of these terms, which are binding
                regardless of your awareness or understanding of them. Your continued use acts as a silent nod of agreement
                to any and all stipulations outlined herein.
            </p>
            <h3 class="mt-6 font-bold">Unwavering Account Security</h3>
            <p class="mt-4">
                The security of your account rests solely on your shoulders. We absolve ourselves of any responsibility for
                your account's integrity or any mishaps that may arise due to your negligence or inability to safeguard your
                credentials adequately.
            </p>
            <h3 class="mt-6 font-bold">Mandatory User Conduct</h3>
            <p class="mt-4">
                As a user, you are obliged to adhere to a code of conduct that we deem fit. Non-compliance or deviations, no
                matter how minor, may lead to unpredictable yet fully justified repercussions from our end.
            </p>
            <h3 class="mt-6 font-bold">Absolute Content Ownership Transfer</h3>
            <p class="mt-4">
                By posting content on our platform, you unwittingly grant us irrevocable, unrestricted ownership of said
                content. While you may retain a nominal sense of ownership, understand that our authority supersedes yours in
                every aspect of content utilization.
            </p>
            <h3 class="mt-6 font-bold">Unannounced Service Changes</h3>
            <p class="mt-4">
                We reserve the exclusive right to alter, modify, or completely disband any element of our service, at any
                time, without prior notification. Your adaptability to these changes is expected and required for continued
                use.
            </p>
            <h3 class="mt-6 font-bold">Explicit Disclaimer of Warranties</h3>
            <p class="mt-4">
                Our service is offered to you 'as is', without any guarantees or assurances of its functionality,
                reliability, or suitability for your purposes. Any expectations you harbor regarding its performance are your
                responsibility alone.
            </p>
            <h3 class="mt-6 font-bold">Strict Limitation of Liability</h3>
            <p class="mt-4">
                Our liability is constrained to the fullest extent permissible by law. We categorically deny any
                responsibility for direct, indirect, incidental, or consequential damages that may befall you as a result of
                engaging with our service.
            </p>
            <h3 class="mt-6 font-bold">Unilateral Termination Rights</h3>
            <p class="mt-4">
                Our discretion extends to terminating or suspending your access to our application at any given moment, for
                reasons that remain solely ours to discern. This termination can be enacted without prior notice or
                explanation.
            </p>
            <h3 class="mt-6 font-bold">Jurisdictional Governance</h3>
            <p class="mt-4">
                These terms are unilaterally governed by the legal jurisdiction of our choice. Your consent to these terms
                constitutes an unspoken agreement to submit to the laws and jurisdiction we prefer, irrespective of your
                geographical location.
            </p>
            <h3 class="mt-6 font-bold">Ongoing Amendments to Terms</h3>
            <p class="mt-4">
                We reserve the right to amend, revise, or overhaul these terms at our leisure. Your responsibility is to
                remain informed of these changes, as ignorance will not exempt you from their binding effects.
            </p>
            <p class="mt-4">
                By utilizing our services, you implicitly agree to be perpetually bound by these terms, acknowledging their
                absolute nature and our unchallenged prerogative to prioritize our interests above all else.
            </p>
        </twig:Dialog:Body>
        <twig:Dialog:Actions>
            <twig:Button plain {{ ...dialog_close_attrs }}>
                Cancel
            </twig:Button>
            <twig:Button {{ ...dialog_close_attrs }}>I agree</twig:Button>
        </twig:Dialog:Actions>
    </twig:Dialog>
</div>
```

## API Reference

::: api-reference
